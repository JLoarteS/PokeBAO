...
def quade_test(*args):
    ...
    rankings_cmp = [r/np.sqrt(k*(k+1)*(2*n+1)*(k-1)/(18.*n*(n+1))) for r in rankings_avg]

    A = np.sum(S[i][j]**2 for i in range(n) for j in range(k))
    B = np.sum(s**2 for s in Sj)/float(n)
    F = (n-1)*B/(A-B)
